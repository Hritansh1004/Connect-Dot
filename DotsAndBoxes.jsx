...
import { getFirestore, doc, getDoc, setDoc, onSnapshot, collection, addDoc, updateDoc, deleteDoc, getDocs, query, where, arrayUnion } from 'firebase/firestore';
import { setLogLevel } from 'firebase/firestore';

// Set Firebase log level to debug for development
setLogLevel('debug');

// Tailwind CSS is assumed to be available. Add this script in your HTML:
...
function App() {
  ...
  useEffect(() => {
    ...
    if (isGameStarted && gameId) {
      gameRef.current = doc(db, 'artifacts', 'dots-and-boxes', 'public', 'games', gameId);
  ...
  const handleCreateGame = async () => {
    ...
    try {
      ...
      const newGameRef = doc(db, 'artifacts', 'dots-and-boxes', 'public', 'games', gameCode);
  ...
  const handleJoinGame = async () => {
    ...
    try {
      const gameRef = doc(db, 'artifacts', 'dots-and-boxes', 'public', 'games', gameId);
      ...
      if (docSnap.exists()) {
        ...
        if (!isAlreadyPlayer) {
          ...
          await updateDoc(gameRef, {
            ...
            scores: { ...gameData.scores, [userId]: 0 },
  ...
  const handleStartGame = async () => {
    ...
    try {
      ...
      await updateDoc(gameRef, {
        isStarted: true,
        message: `The game has started! It's ${players[0]?.initial}'s turn.`
      });
    } catch (error) {
  ...
  const handleConfirmMove = async () => {
    ...
    try {
      ...
      // If a box was captured, the current player gets another turn.
      if (!claimedBox) {
  ...
  const getLineStyle = (line) => {
    ...
    const color = players.find(p => p.id === owner)?.color || 'transparent';

    if (isSelected) {
      return `absolute bg-gray-600 rounded-full transition-all duration-100 ease-in-out z-10`;
    }
    if (isDrawn) {
      return `absolute rounded-full transition-all duration-300 ease-in-out z-10`
    }
    return `absolute rounded-full transition-all duration-100 ease-in-out hover:bg-gray-400 cursor-pointer`;
  };
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                onClick={() => generateDots(5)}
                className={`w-full p-3 rounded-lg font-bold transition-transform transform hover:scale-105 ${gridSize === 5 ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-200 text-gray-700'}`}
              >
                5x5 (Quick)
              </button>
              <button
                onClick={() => generateDots(8)}
                className={`w-full p-3 rounded-lg font-bold transition-transform transform hover:scale-105 ${gridSize === 8 ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-200 text-gray-700'}`}
              >
                8x8 (Standard)
              </button>
              <button
                onClick={() => generateDots(12)}
                className={`w-full p-3 rounded-lg font-bold transition-transform transform hover:scale-105 ${gridSize === 12 ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-200 text-gray-700'}`}
              >
                12x12 (Long)
              </button>
              <button
                onClick={() => generateDots(15)}
                className={`w-full p-3 rounded-lg font-bold transition-transform transform hover:scale-105 ${gridSize === 15 ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-200 text-gray-700'}`}
              >
                15x15 (Epic)
              </button>
            </div>
          ...
          <div className="flex flex-col space-y-4">
            <button
              ...
              className="bg-indigo-600 text-white p-3 rounded-lg font-bold shadow-lg hover:bg-indigo-700 transition-colors"
            >
              Create New Game
            </button>
            <div className="relative">
              <input
                ...
                className="w-full p-3 border-2 border-gray-300 rounded-lg text-center font-bold text-gray-700 uppercase focus:border-indigo-500 focus:ring-indigo-500"
              />
              <button
                onClick={handleJoinGame}
                className="w-full mt-2 bg-green-600 text-white p-3 rounded-lg font-bold shadow-lg hover:bg-green-700 transition-colors"
              >
                Join Existing Game
              </button>
            </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-2xl mb-6 w-full max-w-4xl">
          <div className="flex flex-wrap items-center justify-center space-x-4 mb-6">
            {players.map(player => (
              <div
                key={player.id}
                className={`flex items-center space-x-2 p-3 rounded-xl font-bold text-white shadow-inner transition-all duration-300 ease-in-out transform ${currentTurn === player.id ? 'scale-110 ring-4 ring-offset-2 ring-indigo-500' : ''}`}
          ...
          {isWaitingForOpponent ? (
            <p className="text-center text-gray-600 animate-pulse">Waiting for other players to join...</p>
          ) : (
            <div className="text-center text-xl font-semibold mb-4 text-gray-700">
              {isGameOver ? (
                winner ? `Game Over! ${winnerInitial} wins!` : "Game Over! It's a tie!"
              ) : (
        ...
        <div className="relative" style={{ width: `${gridSize * 40 + 20}px`, height: `${gridSize * 40 + 20}px` }}>
          ...
          {dots.map(dot => (
            <div
              ...
              style={{
                top: `${dot.row * 40 + 10}px`,
                left: `${dot.col * 40 + 10}px`
              }}
            ></div>
          ))}
          ...
          {getPossibleLines(gridSize).map(line => {
            ...
            const owner = lines.find(l => l.id === line.id)?.owner;
            const ownerColor = players.find(p => p.id === owner)?.color;

            let lineStyle = '';
            if (line.type === 'horizontal') {
              lineStyle = `h-2 w-40 top-[${line.start.row * 40 + 10}px] left-[${line.start.col * 40 + 14}px]`;
            } else {
              lineStyle = `w-2 h-40 top-[${line.start.row * 40 + 14}px] left-[${line.start.col * 40 + 10}px]`;
            }

            return (
              <div
                key={line.id}
                className={`absolute rounded-full transition-all duration-300 ease-in-out ${isDrawn ? '' : (currentTurn === userId ? 'hover:bg-gray-400 cursor-pointer' : '')} `}
                style={{
                  ...lineStyle,
                  backgroundColor: isSelected ? myColor : (isDrawn ? ownerColor : 'transparent'),
                }}
                onClick={() => !isDrawn && currentTurn === userId ? handleClickLine(line) : null}
              ></div>
            );
          })}

          {/* Captured Boxes */}
          {capturedBoxes.map(box => (
